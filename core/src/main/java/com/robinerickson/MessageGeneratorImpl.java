...
public class MessageGeneratorImpl implements MessageGenerator {
    ...
    public String getResultMessage() {
        ...
        else if (!game.isValidNumberRange()) return getMessage(INVALID_RANGE);
        else if (game.getRemainingGuesses() == game.getGuessCount()) return getMessage(FIRST_GUESS);
        else {
            String direction = getMessage(LOWER);

            if (game.getGuess() < game.getNumber()) direction = getMessage(HIGHER);
